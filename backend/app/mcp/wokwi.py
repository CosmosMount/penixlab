...
# Wokwi's canonical part type so a round trip does not change board identity.
VELXIO_TO_WOKWI_BOARD["rp2040:rp2040:rpipico"] = "wokwi-pi-pico"
...
def format_wokwi_diagram(
    ...
    """
    board_fqbn = circuit.get("board_fqbn")
    if not isinstance(board_fqbn, str) or not board_fqbn.strip():
        raise ValueError("circuit.board_fqbn is required for Wokwi export")
    wokwi_board_type = VELXIO_TO_WOKWI_BOARD.get(board_fqbn)
    if wokwi_board_type is None:
        raise ValueError(
            f"Unsupported board FQBN for Wokwi export: {board_fqbn}; "
            f"supported FQBNs: {', '.join(sorted(VELXIO_TO_WOKWI_BOARD))}"
        )
