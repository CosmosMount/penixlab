...
from app.services.board_runtime_profiles import PiRuntimeProfile
...
# Keep one typed, immutable representation for both built-in and overlay
# profiles. The mapping-compatible surface preserves the existing command
# builder during this migration.
PI_CONFIGS: dict[str, PiRuntimeProfile] = {
    board_id: PiRuntimeProfile.from_mapping(board_id, raw)
    for board_id, raw in _PI_CONFIGS_RAW.items()
}
...
def register_pi_board_profile(board_type: str, cfg: dict) -> None:
    ...
    """
    PI_CONFIGS[board_type] = PiRuntimeProfile.from_mapping(board_type, cfg)
...
class QemuManager:
    ...
    def start_instance(self, client_id: str, board_type: str,
        ...
        if board_type not in PI_CONFIGS:
            message = (
                f'No Raspberry Pi runtime profile for {board_type!r}; '
                f'known boards: {", ".join(sorted(PI_CONFIGS))}'
            )
            logger.error('start_instance: %s', message)
            asyncio.create_task(callback('error', {'message': message}))
            return
