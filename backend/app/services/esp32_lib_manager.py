...
class EspLibManager:
    ...
    async def start_instance(
        ...
        profile = get_board_runtime_profile(board_type, 'esp32-worker')
        if profile is None:
            await callback('error', {
                'message': f'No ESP32 worker runtime profile for {board_type!r}'
            })
            return
        machine = profile.machine
        lib_path = lib_riscv_path() if profile.architecture == 'riscv32' else lib_xtensa_path()
        config   = json.dumps({
            'lib_path':          lib_path,
