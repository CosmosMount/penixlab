...
from app.services.board_runtime_profiles import get_board_runtime_profile
...
class EspQemuManager:
    ...
    def start_instance(self, client_id: str, board_type: str,
        ...
        if client_id in self._instances:
            ...
            return
        if get_board_runtime_profile(board_type, 'esp32-qemu') is None:
    ...
    async def _boot(self, inst: EspInstance, firmware_b64: str | None,
        ...
        profile = get_board_runtime_profile(inst.board_type, 'esp32-qemu')
        if profile is None:
            await inst.emit('error', {
                'message': f'No ESP32 QEMU runtime profile for {inst.board_type!r}'
            })
            self._instances.pop(inst.client_id, None)
            return
        qemu_bin = QEMU_RISCV32 if profile.architecture == 'riscv32' else QEMU_XTENSA
