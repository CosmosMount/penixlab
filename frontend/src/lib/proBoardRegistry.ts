...
import { registerBoardProfile } from '../boards';
...
export function registerProBoards(defs: ProBoardDef[]): void {
  for (const def of defs) {
    ...
    registerBoardProfile(toBoardProfile(def));
...
function toBoardProfile(def: ProBoardDef): BoardProfile {
  const isPi = !!def.piFamily;
  const family = def.esp32Family ?? (isPi ? 'pi-linux' : 'custom');
  const backend = isPi ? 'pi-qemu' : def.esp32Family ? 'esp32-qemu' : 'custom';
  const artifact = isPi ? 'linux-image' : def.fqbn ? 'bin' : 'none';
  const languages: BoardProfile['capabilities']['languages'] = ['arduino'];
  if (def.supportsMicroPython) languages.push('micropython');
  if (isPi) languages.push('python');
  return {
    id: def.kind,
    displayName: def.label,
    description: def.description,
    fqbn: def.fqbn,
    runtime: { backend, artifact, family },
    power: {
      logicVoltage: 3.3,
      groundPins: ['GND'],
      supplyPins: ['3V3'],
      auxiliaryRails: [{ voltage: 5, pins: ['5V', 'VIN'] }],
    },
    capabilities: {
      support: 'digital',
      languages,
    ...
    protocolRoles: {},
    pinAliases: [],
    resolvePin: (pinName: string) => {
      if (def.pinToNumber) return def.pinToNumber(pinName);
      const normalized = pinName.trim().toUpperCase();
      if (/^(GND|VCC|3V3|3\.3V|5V|VIN|VBUS|VSYS)$/.test(normalized)) return -1;
      const match = /^(?:GPIO|GP|D)?(\d+)$/.exec(normalized);
