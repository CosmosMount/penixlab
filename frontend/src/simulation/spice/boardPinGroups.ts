...
/**
 * Resolve the electrical description without silently treating an unknown
 * board as the default 5 V Arduino. Static boards use the legacy table while
 * dynamically registered boards derive the same shape from their Profile.
 */
export function getBoardPinGroup(boardKind: string): BoardPinGroup | undefined {
  const staticGroup = BOARD_PIN_GROUPS[boardKind as AllBoardKinds];
  if (staticGroup && boardKind !== 'default') return staticGroup;

  const profile = getBoardProfile(boardKind);
  if (!profile) return undefined;
  return {
    vcc: profile.power.logicVoltage,
    gnd: profile.power.groundPins,
    vcc_pins: profile.power.supplyPins,
    aux: profile.power.auxiliaryRails?.[0]
      ? {
          volts: profile.power.auxiliaryRails[0].voltage,
          pins: profile.power.auxiliaryRails[0].pins,
        }
      : undefined,
  };
}
