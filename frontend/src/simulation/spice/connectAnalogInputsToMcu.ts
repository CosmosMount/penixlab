...
function adcPinsForBoard(boardKind: string): Array<{ pinName: string; channel: number }> | undefined {
  return getBoardProfile(boardKind)?.adcPins ?? ADC_PIN_MAP[boardKind as BoardKind];
}
...
export function connectAnalogInputsToMcu(): () => void {
  ...
  function injectVoltagesIntoADC() {
    ...
    for (const board of boards) {
      const adcPins = adcPinsForBoard(board.boardKind);
      if (!adcPins) continue;
      const sim = getBoardSimulator(board.id);
      if (!sim) continue;
      const vMax = getBoardProfile(board.boardKind)?.power.logicVoltage ??
        (board.boardKind.startsWith('esp32') ? 3.3 : 5.0);
