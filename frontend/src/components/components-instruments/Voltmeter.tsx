...
export function Voltmeter({ id }: VoltmeterProps) {
  ...
  const reading = useMemo(() => {
    const groundPins = boards.flatMap((b) => {
      const group = getBoardPinGroup(b.boardKind);
      return group?.gnd.map((pin) => ({ componentId: b.id, pinName: pin })) ?? [];
    });
    const vccPins = boards.flatMap((b) => {
      const group = getBoardPinGroup(b.boardKind);
      return group?.vcc_pins.map((pin) => ({ componentId: b.id, pinName: pin })) ?? [];
    });
    ...
    const auxPins = boards.flatMap((b) => {
      const aux = getBoardPinGroup(b.boardKind)?.aux;
      if (!aux) return [];
