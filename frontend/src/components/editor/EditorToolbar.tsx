...
import { BOARD_SUPPORTS_ESPIDF, BOARD_SUPPORTS_MICROPYTHON, isPiBoardKind, boardDisplayName } from '../../types/board';
...
export const EditorToolbar = ({
  ...
  const reportRun = useCallback(
    (boardKind: BoardKind | undefined, engine?: 'instant' | 'linux') => {
      const fqbn = boardKind ? getBoardFqbn(boardKind) : null;
      void reportRunEvent({
  ...
  const handleCompile = async () => {
    ...
    const fqbn = kind ? getBoardFqbn(kind) : null;
  ...
  const compileAllBoards = async (): Promise<{ ok: number; failed: number }> => {
    ...
    for (const board of boardsList) {
      ...
      const fqbn = getBoardFqbn(board.boardKind);
      if (!fqbn) {
