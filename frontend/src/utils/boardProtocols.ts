...
import { resolveBoardProtocolRole } from '../boards';
...
export function classifyPin(boardKind: string, pinName: string): PinRole {
  ...
  const profileRole = resolveBoardProtocolRole(boardKind, pinName);
  if (profileRole) {
    if (profileRole.kind === 'uart-tx' || profileRole.kind === 'uart-rx') {
      return { kind: profileRole.kind, uart: profileRole.bus };
    }
    return profileRole;
  }
