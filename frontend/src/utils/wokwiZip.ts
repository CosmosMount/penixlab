...
export async function exportToWokwiZip(
  ...
  const boardWokwiType = BOARD_TO_WOKWI_TYPE[boardType];
  const boardId = BOARD_TO_WOKWI_ID[boardType];
  if (!boardWokwiType || !boardId || !getBoardProfile(boardType)) {
    throw new Error(
      `Board ${boardType} has no Wokwi export adapter. ` +
        'Export is currently supported for Arduino Uno/Nano/Mega and Raspberry Pi Pico.',
    );
  }
...
export async function importFromWokwiZip(file: File): Promise<ImportResult> {
  ...
  if (!boardPart) {
    throw new Error(
      'No supported Wokwi board was found in diagram.json. ' +
        'Add an Arduino Uno/Nano/Mega or Raspberry Pi Pico board.',
    );
  }
  const boardType = WOKWI_TYPE_TO_BOARD[boardPart.type];
  if (!getBoardProfile(boardType)) {
    throw new Error(`Wokwi board ${boardPart.type} is not registered in the Board Registry.`);
  }
