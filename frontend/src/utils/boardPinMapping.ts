...
export function boardPinToNumber(boardId: string, pinName: string): number | null {
  ...
  // Static boards resolve through the shared Board Profile. The legacy
  // branches below remain for older overlay-only board kinds until their
  // profiles are registered, but must not override a known profile's null.
  const profile = getBoardProfile(boardId);
  if (profile) return profile.resolvePin(pinName);
