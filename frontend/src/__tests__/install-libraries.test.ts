...
const UNSUPPORTED_BOARD_DIAGRAM = JSON.stringify({
  version: 1,
  author: 'test',
  editor: 'wokwi',
...
describe('importFromWokwiZip — libraries field', () => {
  it('rejects a diagram whose board has no Wokwi adapter instead of using Uno', async () => {
    const zip = new JSZip();
    zip.file('diagram.json', UNSUPPORTED_BOARD_DIAGRAM);
    const blob = await zip.generateAsync({ type: 'blob' });
    const file = new File([blob], 'unsupported.zip', { type: 'application/zip' });
    await expect(importFromWokwiZip(file)).rejects.toThrow('No supported Wokwi board');
  });
